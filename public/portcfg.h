...
extern "C" uint32_t g_uGenericTimerFreq;
#else
extern uint32_t g_uGenericTimerFreq;
#endif
#define PORT_SYSTEM_FREQ (g_uGenericTimerFreq)
...
/**
    ...
    interrupt occurs.
*/
#define PORT_TIMER_FREQ ((uint32_t)(PORT_SYSTEM_FREQ / 100)) // 100 Timer ticks per second...

/**
    Define the default/minimum size of a thread stack
*/
#define PORT_KERNEL_DEFAULT_STACK_SIZE ((K_ADDR)1024)

/**
    Define the size of the kernel-timer thread stack (if one is configured)
*/
#define PORT_KERNEL_TIMERS_THREAD_STACK ((K_ADDR)1024)
